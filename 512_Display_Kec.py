...
VIDEO_STREAM_OUT = '~/tes_final.avi'
jumlahkendaraan = 0
...
def main(args,IDVideo):
    ...
    cap = cv2.VideoCapture('{}.mp4'.format(args.video))
    ...
    while (cap.isOpened()):
        ...
        if count%1800 == 0:
            ...
            if len(kec_menit) == 0:
                ...
                kec_avg = (sum(kec_menit)/len(kec_menit))/60*3.6
                print('Avg Speed : ',kec_avg)
        ...
        kecepatan_text = 'Kecepatan rata-rata: {} km/jam'.format(kec_avg)
        kepadatan_text = 'Kepadatan : {} kendaraan/km'.format(kepadatan*1000)
        #cv2.putText(frame,counter_textH,(5,100), font, fontscale,(0,255,0),font_thickness,cv2.LINE_AA)
        cv2.putText(frame,kecepatan_text,(5,100), font, fontscale,(0,0,255),font_thickness,cv2.LINE_AA)
        cv2.putText(frame,kepadatan,(5,200), font, fontscale,(0,0,255),font_thickness,cv2.LINE_AA)
        
        if writer is None:
        # Initialize our video writer
            fourcc = cv2.VideoWriter_fourcc(*"XVID")
            writer = cv2.VideoWriter(VIDEO_STREAM_OUT, fourcc, 30,(1920, 1080), True)
   
            # Write the output frame to disk
        writer.write(frame)#cv2.imshow('video',frame)
